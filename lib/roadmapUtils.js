...
// e.g. "9 items + 4 feature phases" — the phase part only appears when
// at least one item in the list actually has phases.
export function countLabel(itemsList) {
  ...
  if (phaseCount === 0) return itemPart;
  return `${itemPart} + ${phaseCount} feature phase${phaseCount === 1 ? '' : 's'}`;
}
