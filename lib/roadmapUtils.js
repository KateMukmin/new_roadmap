...
export const STATUS_LABEL = { now: 'Now', next: 'Next', later: 'Later', released: 'Released' };

const STATUS_WEIGHT = { now: 0, next: 1, later: 2, released: 3 };
...
// e.g. "9 items + 4 feature phases, 3 items linked" — the phase clause
// only appears when the list has phases, same for the linked clause.
export function countLabel(itemsList) {
  const itemCount = itemsList.length;
  const phaseCount = itemsList.reduce((sum, i) => sum + ((i.phases && i.phases.length) || 0), 0);
  const linkedCount = itemsList.filter((i) => i.description && i.description.trim()).length;

  const parts = [`${itemCount} item${itemCount === 1 ? '' : 's'}`];
  if (phaseCount > 0) parts.push(`${phaseCount} feature phase${phaseCount === 1 ? '' : 's'}`);
  let label = parts.join(' + ');
  if (linkedCount > 0) label += `, ${linkedCount} item${linkedCount === 1 ? '' : 's'} linked`;
  return label;
}
...
// Splits text on any http(s) URL and returns an array of plain strings and
// {url} markers, so the caller can render the URLs as real links.
export function linkifyParts(text) {
  return (text || '').split(/(https?:\/\/[^\s]+)/g).map((part) =>
    /^https?:\/\//i.test(part) ? { url: part } : part
  );
}
